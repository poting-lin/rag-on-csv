...
''')
        ...
        result = subprocess.run(
            ["python", temp_script], cwd=self.project_root, capture_output=True, text=True, timeout=timeout
        )
            ...
            if match:
                ...
                if expectations.get("contains"):
                    ...
                    assert occurrences >= expectations["min_matches"], (
                        f"Expected at least {expectations['min_matches']} occurrences "
                        f"of '{first_expected}', but found {occurrences}"
                    )
    ...
    def test_festival_query_sound_measurements(self):
        ...
        self.assert_response_matches_expectations(
            output, EXPECTED_RESPONSES["sound_measurements"]["all_records_festival"]
        )
    ...
    def test_urban_park_location_query(self):
        ...
        # Check that the output matches expectations
        self.assert_response_matches_expectations(
            output, EXPECTED_RESPONSES["sound_measurements"]["urban_park_location"]
        )
    ...
    def test_nonexistent_query(self):
        ...
        no_matches_indicators = ["No matches found", "0 records", "0 matches", "No data available", "couldn't find"]
        ...
        assert has_no_matches_indicator or no_high_values, (
            "Output should indicate no records were found or contain no high values"
        )
    ...
    def test_complex_query_decomposition(self):
        ...
        output = self.run_query("sound_measurements.csv", "analyze all records with EventType Event")
...
class TestMultipleDatasetIntegration:
    ...
    def test_lab_data_query(self):
        ...
        output = self.test_instance.run_query("lab_data.csv", "records with Equipment containing Optical")

        # Check that the output matches expectations
        self.test_instance.assert_response_matches_expectations(
            output, EXPECTED_RESPONSES["lab_data"]["optical_equipment"]
        )
    ...
    def test_bookshop_query(self):
        ...
        output = self.test_instance.run_query("bookshop.csv", "all records with Category Fiction")
...
class TestErrorHandling:
    ...
    def test_invalid_csv_path(self):
        ...
        error_indicators = ["error", "not found", "no such file", "cannot open", "failed to load", "couldn't load"]
    ...
    def test_empty_query(self):
        ...
        assert "question" in output.lower() or "query" in output.lower(), "Output should prompt for a question"
